...
// WalkTarget visits the receiver with the provided callback.
func WalkEmbedsTarget(x EmbedsTarget, fn TargetWalkerFn) (_ EmbedsTarget, changed bool, err error) {
	var y e.Ptr
	y, changed, err = targetEngine.Execute(fn, e.TypeId(TargetTypeEmbedsTarget), e.Ptr(&x))
	if err != nil {
		return nil, false, err
	}
	return *(*EmbedsTarget)(y), changed, nil
}
